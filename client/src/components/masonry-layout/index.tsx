...
import { pxTransform, getSystemInfo } from "@tarojs/taro";
...
export const MasonryLayout = () => {
  ...
  const onImageLoad = async (e) => {
    ...
    let imgWidth = (await getWindowWidth()) / 2; //图片设置的宽度
  ...
  const handleImageLoad = () => {
    ...
    for (let i = 0; i < goodsList.length; i++) {
      ...
      ImageLoadList[i].img = goodsList[i].img;
  ...
  //   setData([...data, 1, 2, 3, 4, 5, 6]);
  // }, [data]);
  // const { data1, data2 } = getImages();
  return (
    ...
    <Container>
      ...
      <Column>
        {goodsRight.map((v, index) => {
          return (
            <Item key={index} src={v.img} style={v.imgStyle} mode="widthFix" />
          );
        })}
      </Column>
    </Container>
